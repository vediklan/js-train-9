...
function findElementGreaterThan(arr, num) {
  ...
  const found = arr.find((item) => item > num);
...
function findIndexByProperty(arr, name) {
  ...
  const findIndex = arr.findIndex((item) => item.name === name);
...
function findLastEvenNumber(arr) {
  ...
  const evenNumber = arr.findLast((item) => item % 2 === 0);
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const sliceArr = arr.slice(startIdx);
...
function getPositiveArrayValues(arr) {
  ...
  const positiveArr = arrValues.filter(function (item) {
    return item > 0;
  });
...
function removeAndDouble(arr, startIdx) {
  ...
  const spliceArr = arr.splice(0, startIdx);
  // За допомогою методу map() подвоюємо кожен елемент масиву.
  const dubleItemArr = spliceArr.map((item) => item * 2);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const squaringArr = flatArr.map((item) => item ** 2);
  // За допомогою методу filter() фільтруємо елементи, які більше за порогове значення.
  const filterArr = squaringArr.filter((item) => item > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sumArr = sliceArr.reduceRight(
    (accumulator, currentValue) => accumulator + currentValue,
    0
  );
...
function wordLengths(arr) {
  ...
  const mapArr = arr.map((word) => word.length);
...
function flattenArray(arr) {
  ...
  const flattenedArray = arr.flat(4);
...
function findDuplicateElements(arr) {
  ...
  const duplicateElements = arr.filter((elem, index) => {
...
function capitalizeWords(arr) {
  ...
  const result = arr.map((word) => {
    const modifWord = word.charAt(0).toUpperCase() + word.slice(1);
    return modifWord;
  });
...
function calculateTotalPrice(arr) {
  ...
  const sumPrices = arr.reduce((accomPrice, currentPrice) => {
    // Додаємо ціну товару до загальної суми та повертаємо результат
    return accomPrice + currentPrice.price;
  }, 0);
